...
public readonly struct Layer : IEquatable<Layer>, IComparable<Layer> {
    ...
    public Color Color => Metadata?.Color ?? new Color();
    ...
    public static Color GetLayerColor(string layerName) => LayerMetadata.Get(layerName)?.Color ?? new Color();

    public static Layer FromByte(byte value) => new(value);

    public static implicit operator Layer(byte value) => new(value);

    public static implicit operator byte(Layer coord) => coord._value;
